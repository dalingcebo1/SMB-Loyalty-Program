```markdown
# Azure Deployment Runbook (procedural)
...
(Imported from root `AZURE_DEPLOYMENT_STEPS.md`)

This runbook standardizes production deployment on Azure using Container Apps, ACR, and GitHub Actions.

Quick links:
- Secrets & GitHub Actions: see `docs/deployment/secrets.md`
- Rollback & scaling: see `docs/deployment/rollback.md`

(Keep the full procedural content in the original file for now; consider full migration in next pass.)
