...
const CarWashDashboard: React.FC = () => {
  ...
  const completedList = history.filter((w: any) => w.status === 'ended' && w.ended_at);
  const inProgressList = history.filter((w: any) => w.status === 'started');
  const totalHistory = history.length;
  const completedCount = completedList.length;
  const inProgressCount = inProgressList.length;
  const avgDurationMs = completedCount > 0
    ? completedList.reduce(
        (sum: number, w: any) => sum + (new Date(w.ended_at).getTime() - new Date(w.started_at).getTime()),
        0
      ) / completedCount
    : 0;
...
// This page has been moved to src/features/staff/pages/CarWashDashboard.tsx
