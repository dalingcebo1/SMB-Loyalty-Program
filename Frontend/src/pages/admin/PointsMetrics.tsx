...
const PointsMetrics: React.FC = () => {
  ...
  // ensure URL has date params if missing
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    api.get(`/analytics/points/details?start_date=${startDate}&end_date=${endDate}`)
  ...
  if (loading) {
    return <p>Loading points metrics...</p>;
  }
  if (!data) {
    return <p>No points metrics to display.</p>;
  }
  ...
  return (
    <div className="p-4">
      ...
      <h2 className="text-xl font-semibold mb-2">Points Metrics</h2>
