...
class Settings(BaseSettings):
    ...
    # NOTE: pydantic v2 settings: use alias to map environment variable names (mypy-safe; "env" kw was v1).
    jwt_secret: str = Field("dev_jwt_secret", alias="JWT_SECRET")
    algorithm: str = "HS256"
    access_token_expire_minutes: int = 60 * 24
    reset_secret: str = Field("dev_reset_secret", alias="RESET_SECRET")
    ...
    firebase_credentials_json: Optional[str] = Field(None, alias="FIREBASE_CREDENTIALS_JSON")
    ...
    rate_limit_public_meta_capacity: int = Field(60, alias="RATE_LIMIT_PUBLIC_META_CAPACITY")
    rate_limit_public_meta_window_seconds: int = Field(60, alias="RATE_LIMIT_PUBLIC_META_WINDOW")
    rate_limit_user_tenant_capacity: int = Field(30, alias="RATE_LIMIT_USER_TENANT_CAPACITY")
    rate_limit_user_tenant_window_seconds: int = Field(60, alias="RATE_LIMIT_USER_TENANT_WINDOW")
    rate_limit_global_capacity: int = Field(120, alias="RATE_LIMIT_GLOBAL_CAPACITY")
    rate_limit_global_window_seconds: int = Field(60, alias="RATE_LIMIT_GLOBAL_WINDOW")
    enable_rate_limit_overrides: bool = Field(True, alias="ENABLE_RATE_LIMIT_OVERRIDES")  # disable in prod to lock config
    enable_rate_limit_penalties: bool = Field(True, alias="ENABLE_RATE_LIMIT_PENALTIES")
    enable_job_queue: bool = Field(False, alias="ENABLE_JOB_QUEUE")  # in-process queue generally off in prod
    enable_metrics_endpoint: bool = Field(True, alias="ENABLE_METRICS")
    # Enable loading CORS allowed origins from DB table when env variables are absent
    enable_db_cors_allowlist: bool = Field(False, alias="ENABLE_DB_CORS_ALLOWLIST")

    # --- Dev / safety feature flags ---
    environment: str = Field("development", alias="ENVIRONMENT")  # e.g. development|staging|production
    enable_dev_dangerous: bool = Field(True, alias="ENABLE_DEV_DANGEROUS")  # master switch for destructive ops
    enable_dev_jobs: bool = Field(True, alias="ENABLE_DEV_JOBS")
    enable_dev_rate_limits: bool = Field(True, alias="ENABLE_DEV_RATE_LIMITS")
    enable_dev_audit_view: bool = Field(True, alias="ENABLE_DEV_AUDIT_VIEW")
